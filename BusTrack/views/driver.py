...
def add_gps():
    ...
    if 'lat' in js and 'lon' in js:
        ...
        if j_id is None or j_id is '':
            ...
            # update gps table
            # TODO: add gps directly into location table
...
def drop_attendance():
    ...
    if 'kid_ids' in js and 'lat' in js and 'lon' in js:
        ...
        if j_id is None or j_id is '':
            ...
            ids_valid = list(ids_set & ids_db_set)  # intersection of kid ids
            for id in ids_valid:
                Attendance.update_drop_attendance(id, gps, time)
            return jsonify(status='ok', message='Attendance taken')
    else:
        return jsonify(status='error', message='incorrect request')
...
# TODO:generate url to send kid photo
...
def get_kid_photo():
    if 'kid_id'in request.args:
        ...
        token=request.args['token']
        img_name=Kid.get_image(token,k_id)
        if img_name is None:
            abort(404)
        else:
            ...
            try:
                return send_file(final_image,mimetype='image/*')
            except:
                abort(404)
    else:
        abort(404)

# get information about completed ride  by this driver
'''
